...
##97.5% Quant = []
## model_list = [] ## NOTE: New stuff. placeholder for loaded models [TEST]
...
## this line is to check if a model existed. Still need test
## For now, make sure the model existed BEFORE running the model
    file_path = namamodel[k]
    check_file_and_operate(file_path, simbol, waktuframe)
    ## isi_model = load_pickle('arimas/'+file_path+'.pkl') ## load individual model [TEST]
    ## model_list.append(isi_model) ## save it to model placeholder list [TEST]

print("Ready to Predict!") ## no hello :(

while True: ## need better while here maybe
...
##                datas[k].append([saatini[k], pred[k,0], close_np[39]])
            ...
            if tr_param == 'c':
                cetak_param = f'model: ARIMA({ar_param},{df_param},{ma_param}) with constant'
            elif tr_param == 't':
                cetak_param = f'model: ARIMA({ar_param},{df_param},{ma_param}) with trend'
            elif tr_param == 'n':
                cetak_param = f'model: ARIMA({ar_param},{df_param},{ma_param})'
